...
/**
 * Tests for {@link GreaterQueryBuilder}
 */
@RunWith(JUnit4.class)
public class GreaterThanQueryBuilderTest extends AbstractQueryBuilderTest {

    @Test
    public void testGreaterThanFilter() throws Exception {
        GreaterThanFilter<Integer> filter = new GreaterThanFilter<>();
        filter.setValue(4);

        List<SampleEntity> result = QueryTestUtils.queryEntity(new GreaterThanQueryBuilder(), SampleEntity_.id, filter);
        assertEquals(2, result.size());
    }

    @Test
    public void testEmptyGreaterThanFilter() throws Exception {
        GreaterThanFilter<String> filter = new GreaterThanFilter<>();

        List<SampleEntity> result = QueryTestUtils.queryEntity(new GreaterThanQueryBuilder(), SampleEntity_.name, filter);
